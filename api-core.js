...
async function fetchAllFromAPI() {
  try {
    ...
    // نجيب فقط أول 3 أقسام (الأهم)
    const mainCategories = data.data.slice(0, 3);
    let allSeries = [];

    // نجيب كل الأقسام بالتوازي (أسرع)
    ...
    console.log('✅ تم جلب ' + allSeries.length + ' مسلسل من API');
    return allSeries;
  } catch(e) {
    console.log('❌ فشل جلب API:', e.message);
    return null;
  }
}

// ========== دالة استخراج رقم الحلقة من أي صيغة ==========
function extractEpisodeNumber(name) {
  if (!name) return null;
  
  // "الحلقة 5" أو "حلقة 5"
  let match = name.match(/حلقة\s*(\d+)/i);
  if (match) return match[1];
  
  // "Episode 5" أو "Ep 5" أو "E5"
  match = name.match(/(?:Episode|Ep|E)\s*#?\s*(\d+)/i);
  if (match) return match[1];
  
  // "Ep#5" أو "EP#5"
  match = name.match(/Ep#(\d+)/i);
  if (match) return match[1];
...
async function fetchEpisodesFromAPI(seriesUrl) {
  try {
    console.log('📡 جاري جلب الحلقات من API للرابط:', seriesUrl);
    const res = await fetch(API_BASE + '?action=episodes&url=' + encodeURIComponent(seriesUrl));
    const data = await res.json();
    console.log('📥 استجابة API للحلقات:', data);
    ...
    if (data.status && data.data && data.data.length > 0) {
      ...
      console.log('✅ تم تحويل ' + episodes.length + ' حلقة');
      return episodes;
    }
    console.log('⚠️ API رجع بيانات بس ما فيه حلقات');
    return null;
  } catch(e) {
    console.log('❌ فشل جلب الحلقات من API:', e.message);
    return null;
  }
}
